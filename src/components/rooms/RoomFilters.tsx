...
const RoomFilters: React.FC<RoomFiltersProps> = ({ onFilterChange }) => {
  ...
  const CustomInput = React.forwardRef(({ value, onClick }: any, ref: any) => (
    <button
      ...
      onClick={onClick}
      ref={ref}
      className="w-full px-5 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-teal-500 focus:border-teal-500 bg-white text-left"
    >
      {value || 'Select date'}
    </button>
  ));
